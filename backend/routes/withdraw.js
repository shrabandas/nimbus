...
function register(router) {
  ...
  router.post('/api/withdraw', requireAuth, async (req, res) => {
    try {
      ...
      const withdrawal = await db.prepare('SELECT * FROM withdrawals WHERE id = ?').get(info.lastInsertRowid);
  ...
  router.get('/api/withdraw/mine', requireAuth, async (req, res) => {
  ...
  router.get('/api/withdraw/admin/all', requireAuth, requireAdmin, async (req, res) => {
    const rows = await db.prepare(`
  ...
  router.post('/api/withdraw/admin/:id/decide', requireAuth, requireAdmin, async (req, res) => {
    const { decision } = req.body;
    if (!['approved', 'rejected'].includes(decision)) {
      return res.json({ error: "Decision must be 'approved' or 'rejected'" }, 400);
    }
    const withdrawal = await db.prepare('SELECT * FROM withdrawals WHERE id = ?').get(req.params.id);
    ...
    const updated = await db.prepare('SELECT * FROM withdrawals WHERE id = ?').get(req.params.id);
