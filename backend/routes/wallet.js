...
async function addUserEarning({ userId, amount, bonusName, adminId }) {
  ...
  const targetUser = await db.prepare('SELECT id, is_admin FROM users WHERE id = ?').get(Number(userId));
  if (!targetUser) {
...
async function deductUserEarning({ userId, amount, reason, adminId }) {
  const numAmount = Number(amount);
...
async function computePortfolio(userId) {
  const approvedDeposits = await db.prepare(
  ...
  const planPurchasesRow = await db.prepare(
    'SELECT COALESCE(SUM(price_usdt),0) AS s FROM plan_purchases WHERE user_id = ?'
  ).get(userId);
  const planPurchases = planPurchasesRow.s;
  ...
  const pendingDeposits = await db.prepare(
...
function register(router) {
  ...
  router.post('/api/wallet/deposit', requireAuth, async (req, res) => {
    try {
      ...
      const deposit = await db.prepare('SELECT * FROM deposits WHERE id = ?').get(info.lastInsertRowid);
      res.json({
  ...
  router.get('/api/wallet/admin/deposits', requireAuth, requireAdmin, async (req, res) => {
  ...
  router.post('/api/wallet/admin/deposits/:id/decide', requireAuth, requireAdmin, async (req, res) => {
    const { decision } = req.body;
    if (!['approved', 'rejected'].includes(decision)) {
      return res.json({ error: "Decision must be 'approved' or 'rejected'" }, 400);
    }
    const deposit = await db.prepare('SELECT * FROM deposits WHERE id = ?').get(req.params.id);
    ...
    const updated = await db.prepare('SELECT * FROM deposits WHERE id = ?').get(req.params.id);
