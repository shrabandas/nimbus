...
function register(router) {
  ...
  router.get('/api/admin/plan-purchases', requireAuth, requireAdmin, (req, res) => {
  ...
  router.get('/api/admin/earnings', requireAuth, requireAdmin, (req, res) => {
  ...
  router.post('/api/admin/earnings', requireAuth, requireAdmin, (req, res) => {
    try {
  ...
  router.post('/api/admin/earnings/deduct', requireAuth, requireAdmin, (req, res) => {
    try {
      const { user_id, amount, reason } = req.body;
      const user = db.prepare('SELECT id FROM users WHERE id = ?').get(Number(user_id));
      if (!user) return res.json({ error: 'User not found' }, 404);

      const record = deductUserEarning({
        userId: user.id,
        amount,
        reason,
        adminId: req.user.id,
  ...
  router.post('/api/admin/bonus-redemptions/:id/decide', requireAuth, requireAdmin, (req, res) => {
    const { decision } = req.body;
    if (!['approved', 'rejected'].includes(decision)) {
      return res.json({ error: "Decision must be 'approved' or 'rejected'" }, 400);
    }

    const redemption = db.prepare('SELECT * FROM bonus_redemptions WHERE id = ?').get(req.params.id);
    ...
    const updated = db.prepare('SELECT * FROM bonus_redemptions WHERE id = ?').get(req.params.id);
