...
function register(router) {
  ...
  router.post('/api/plans/buy', requireAuth, async (req, res) => {
    try {
      ...
      const buyer = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
      if (buyer && buyer.referred_by) {
        const referrer = await db.prepare('SELECT * FROM users WHERE id = ?').get(buyer.referred_by);
        const purchaseCountRow = await db.prepare(
          'SELECT COUNT(*) AS c FROM plan_purchases WHERE user_id = ?'
        ).get(referrer ? referrer.id : 0);
        const hasPurchasedPlan = purchaseCountRow.c > 0;
  ...
  // GET /api/plans/mine
  router.get('/api/plans/mine', requireAuth, async (req, res) => {
